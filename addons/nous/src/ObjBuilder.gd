...
func export_scene_to_obj(scene: Node, out_path = null) -> void:
	...
	for info in mesh_info_array:
		MeshUtils.ProcessMesh(info[MESHINFO.MESH], info[MESHINFO.TRANSFORM], Nous.Settings.scale_factor)
