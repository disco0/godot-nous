...
func _ready() -> void:
	...
	buildable_searcher = FGDEntitySceneSearch.new(Nous.fgd)
...
func resolve_source_res(ent_info: FGDEntityObjectData):
	...
	var defs := Nous.fgd.get_entity_definitions()
...
func inspect_item_fgd(item: TreeItem) -> void:
	...
	if is_instance_valid(data):
		...
		if not orig:
			...
			Nous.editor.inspect_object(orig, "", true)

			var inspector_tab = Nous.editor.get_tree().root.find_node('Inspector', true, false)
