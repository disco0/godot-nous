...
var cancelled := false
var tb_game_dir := Nous.Settings.tb_game_dir
...
func _init_defaults() -> void:
	tb_game_dir = Nous.Settings.tb_game_dir
	if is_inside_tree():
		scene_tree = get_tree()
	blocking = false
...
func save_meshes_to_obj(meshes: Array, object_name: String, obj_path: String = "", separate_objects := false):
	...
	for mesh_item in meshes:
		...
		if mesh_item[MESHINFO.TRANSFORM] is Vector3:
			offset = mesh_item[MESHINFO.TRANSFORM]
		else:
			offset = Vector3.ZERO
...
#
# @NOTE: Couldn't get this to work previously, now that builder calls are properly awaited try and
#        reintegrate this
...
func resolve_spatialmat_albedo(mat: SpatialMaterial) -> Image:
	...
	# If found create texture with solid color
