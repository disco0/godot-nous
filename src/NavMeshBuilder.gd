...
#
# Originally planned to replace deep scene cloning with EditorNavigationMeshGenerator's group based
...
func set_genmode_flag(flag: int, value: bool) -> void:
	...
	dprint('Current value of %s => %s' % [ flag_name, current ], 'set_genmode_flag')
	...
	dprint('Updated value of %s => %s' % [ flag_name, updated ], 'set_genmode_flag')
...
func InstanceNavMeshBase(type: int = MESH_BASE_TYPE.DEFAULT) -> NavigationMesh:
	...
	match type:
		...
		MESH_BASE_TYPE.DEFAULT:
			dprint('Building default NavigationMesh instance', 'InstanceNavMeshBase')
		...
		MESH_BASE_TYPE.GROUP:
			dprint('Building NavigationMesh instance for group based build', 'InstanceNavMeshBase')
...
class EditedScene:
	...
	func _get_cached_navmesh_filepath() -> String:
		...
		if _scene_path:
			...
			get_script().dprint('Failed to resolve nav cache path.', "EditedScene:cached_navmesh_path")
			return ""
	...
	func _get_qodot_copy_filepath() -> String:
		...
		if _scene_path:
			...
			get_script().dprint('Failed to resolve qodot copy path.', "EditedScene:qodot_copy_filepath")
			return ""
...
func validate_level_nav_node(level: Spatial) -> NavigationMeshInstance:
	...
	if is_instance_valid(nav_mesh_inst_node):
		...
		if level.get_node('Navigation'):
			dprint('- Adding missing NavigationMeshInstance node', 'validate_level_nav_node')
			...
			dprint('[WARNING] Missing both Navigation and Navigation/NavigationMeshInstance in level scene tree.', 'validate_level_nav_node')
			return null
...
func get_level_node_to_process() -> Node:
	...
	if not is_instance_valid(level):
		dprint('[WARNING] Failed to current Level scene root node', 'get_level_node_to_process')
		return null

	dprint('- Acquired Level node @%s' % [ level ], 'get_level_node_to_process')
...
func clear_nav_group_in_scene(node = get_level_node_to_process()) -> bool:
	if not is_instance_valid(node):
		dprint('[WARNING] Failed to current Level scene root node', 'clear_nav_group_in_scene')
		return false

	var grouped_nodes = node.get_tree().get_nodes_in_group(NAVMESH_SOURCE_GROUP_NAME)
	dprint('Clearing %s group from %s scene nodes' % [
...
func rebuild_nav_via_group() -> bool:
	...
	if not is_instance_valid(level):
		dprint('[WARNING] Failed to current Level scene root node', 'rebuild_nav_via_group')
		return false

	var nav_mesh_inst_node = validate_level_nav_node(level)
	if not is_instance_valid(nav_mesh_inst_node):
		dprint('[WARNING] Failed NavigationMeshInstance validation', 'rebuild_nav_via_group')
		level = null
		return false
	...
	if not is_instance_valid(map_node):
		dprint('[WARNING] Failed get QodotMap node', 'rebuild_nav_via_group')
		level = null
		return false

	# Remove existing child node of QodotMap in NavMeshInstance if found
	# @NOTE: Clears everything out for now
	dprint('Removing any existing children in NavigationMeshInstance', 'rebuild_nav_via_group')
	...
	match target_mode:
		TARGETMODE.ENT_REGEX:
			dprint('Searching for entities with names matching %s' % [ ent_filter.regex.get_pattern() ])
			...
			for ent_node in map_node.get_children():
				...
				if add:
					dprint('    -> %s' % [ ent_node ], 'rebuild_nav_via_group')
		...
		TARGETMODE.WORLDSPAWN:
			...
			for ent_node in map_node.get_children():
				if (ent_node.name as String).matchn('*worldspawn*'):
					dprint('    -> [WORLDSPAWN] %s' % [ ent_node ], 'rebuild_nav_via_group')
					update_node_nav_group(ent_node, true)

	dprint('- Generating navmesh', 'rebuild_nav_via_group')
	...
	match gen_mode:
		GENMODES.STATIC_COLLIDERS:
			dprint('  - Targeting collision shapes', 'rebuild_nav_via_group')
		...
		GENMODES.MESH_INSTANCE:
			dprint('  - Targeting mesh instances', 'rebuild_nav_via_group')
		...
		_:
			if gen_mode == GENMODES.STATIC_COLLIDERS | GENMODES.MESH_INSTANCE:
				dprint('  - Targeting collision shapes and mesh instances', 'rebuild_nav_via_group')
	...
	if grouping_mode == NODE_GROUPING_MODE.CHILDREN:
		dprint('  - Entity children have been grouped directly, updating navmesh geometry mode to SOURCE_GEOMETRY_GROUPS_EXPLICIT', 'rebuild_nav_via_group')
	...
	var meshgen = EditorNavigationMeshGenerator.new()

	dprint('  - Clearing existing NavigationMesh', 'rebuild_nav_via_group')
	...
	dprint('  - Attemping immediate bake using %s nodes in group %s' % [
			grouped_node_count, NAVMESH_SOURCE_GROUP_NAME ], 'rebuild_nav_via_group')

	dprint('  - Starting bake', 'rebuild_nav_via_group')
	meshgen.bake(nav_mesh_inst_node.navmesh, level)

	dprint('  - Bake complete, generated mesh with %s polygons' % [ nav_mesh_inst_node.navmesh.get_polygon_count() ], 'rebuild_nav_via_group')
...
func collect_map_nav_collision_shapes(map_node: QodotMap, node_array: Array = [ ]) -> Array: # CollisionShape[]
	...
	for ent in map_node.get_children():
		if ent_filter.is_processed_in_navmesh(ent):
			dprint(' - Collecting CollisionShapes for @%s' % [ ent.name ], 'collect_map_nav_collision_shapes')
...
func collect_map_nav_mesh_instances(map_node: QodotMap, node_array: Array = [ ]) -> Array: # MeshInstance[]
	...
	for ent in map_node.get_children():
		if ent_filter.is_processed_in_navmesh(ent):
			dprint(' - Collecting MeshInstance for @%s' % [ ent.name ], 'collect_map_nav_mesh_instances')
			collect_entity_mesh_instances(node_array, ent)
		else:
			pass
...
func build_nav_mesh_instance_child(map_node: QodotMap, level: Node = get_level_node_to_process()) -> QodotMap:
	...
	dprint('Duplicated QodotMap base node, clearing its children.', 'build_nav_mesh_instance_child')
	...
	dprint('Building NavigationMeshInstance Map Proxy', 'build_nav_mesh_instance_child')
	...
	dprint("Recursively updating NavigationMeshInstance Map Proxy children's owners to scene root", 'build_nav_mesh_instance_child')
	recursive_own(nav_map_node, level)

	dprint('- Generated navmesh QodotEntity child, with %s children:' % [
...
func save_nav_mesh_resource(navmesh: NavigationMesh, path: String) -> bool:
	...
	if is_instance_valid(navmesh):
		if navmesh.get_polygon_count() == 0:
			dprint('[WARNING] navmesh is valid but has no vertices, returning success but skipping save to resource.', 'save_nav_mesh_resource')
			return true

		var globalized_cache_path: String = ProjectSettings.globalize_path(path)
		if path and globalized_cache_path:
			dprint('- Saving new navmesh to file cache <%s>' % [ path ], 'save_nav_mesh_resource')
			var save_err = -1
			save_err = ResourceSaver.save(globalized_cache_path, navmesh)
			if save_err != OK:
				dprint('  - Saving copy of new NavigationMesh failed, error #%s' % [ save_err ], 'save_nav_mesh_resource')
				return false

	else:
		dprint('[WARNING] navmesh passed is not valid instance.', 'save_nav_mesh_resource')
		return false
...
func save_level_to_resource(map_navmesh_child, nav_mesh_inst_node, level, map_node):
	...
	dprint('- Saving navigation copy QodotMap to resource <%s>' % [ qodot_clone_path ], 'rebuild_nav_via_gen')
	var scene = PackedScene.new()
	dprint('  - Packing QodotMap node', 'rebuild_nav_via_gen')
	scene.pack(map_navmesh_child)
	dprint('  - Saving resource to <%s>' % [ qodot_clone_path ], 'rebuild_nav_via_gen')
	# @TODO test ResourceSaver.FLAG_RELATIVE_PATHS flag
	var save_err = ResourceSaver.save(qodot_clone_path, scene)
	if save_err != OK:
		dprint('  - Save failed, error #%s' % [ save_err ], 'rebuild_nav_via_gen')
	...
	dprint('  - Save Complete.', 'rebuild_nav_via_gen')

	dprint('- Loading map copy from created resource.', 'rebuild_nav_via_gen')
	...
	dprint('- Building navmesh', 'rebuild_nav_via_gen')
	...
	# meshgen.clear(nav)
	dprint('  - Attemping immediate bake', 'rebuild_nav_via_gen')
	meshgen.bake(nav_mesh_inst_node.navmesh, nav_mesh_inst_node)
	dprint('  - End of immediate bake', 'rebuild_nav_via_gen')

	dprint('- Removing QodotMap reference from NavigationMeshInstance', 'rebuild_nav_via_gen')
...
func rebuild_nav_via_gen() -> bool:
	var level =  get_level_node_to_process()
	if not is_instance_valid(level):
		dprint('[WARNING] Failed to current Level scene root node', 'rebuild_nav_via_gen')
		return false

	dprint('- Acquired Level node @%s' % [ level ], 'rebuild_nav_via_gen')
	dprint('  Level resource path: <%s>' % [ level.get_tree().edited_scene_root.filename ], 'rebuild_nav_via_gen')

	var nav_mesh_inst_node = validate_level_nav_node(level)
	if not is_instance_valid(nav_mesh_inst_node):
		dprint('[WARNING] Failed NavigationMeshInstance validation', 'rebuild_nav_via_gen')
		return false

	# Get map node
	var map_node = level.get_node('QodotMap')
	if not is_instance_valid(map_node):
		dprint('[WARNING] Failed get QodotMap node', 'rebuild_nav_via_gen')
		return false

	dprint('Removing any existing children in NavigationMeshInstance', 'rebuild_nav_via_gen')
	for child in nav_mesh_inst_node.get_children():
		nav_mesh_inst_node.remove_child(child)

#	map_node.set('visible', false)

	dprint('- Generating trimmed QodotEntity for NavigationMeshInstance:', 'rebuild_nav_via_gen')
	var map_navmesh_child := build_nav_mesh_instance_child(map_node, level)

	dprint('- Creating NavigationMesh for NavigationMeshInstance', 'rebuild_nav_via_gen')
	...
	dprint('- TOOD: Restore packing generated CollisionShapes version of map.', 'rebuild_nav_via_gen')

	map_node.set('visible', true)

	dprint('- Complete.', 'rebuild_nav_via_gen')
	return true

func rebuild_nav(mode := bake_mode) -> bool:
	match mode:
		NAVBAKEMODES.GROUP:
			dprint('Rebuilding with group tagging method', 'rebuild_nav')
			return rebuild_nav_via_group()

		NAVBAKEMODES.INSTANCE_CHILDREN:
			dprint('Rebuilding with generation method', 'rebuild_nav')
			return rebuild_nav_via_gen()

		_:
			dprint('Defaulting to group tagging method', 'rebuild_nav')
			return rebuild_nav_via_group()
...
func navcache_load_cached() -> void:
	if not is_instance_valid(edited):
		dprint('Invalid edited member.', 'navcache_load_cached')

	var nav_cached_path = edited.cached_navmesh_path

	var level = get_level_node_to_process()
	if not is_instance_valid(level):
		dprint('[WARNING] Failed to get dereferenced copy of Level parent node', 'navcache_load_cached')
		return

	# Load cached navmesh scene
	var nav_cache_tscn = load(nav_cached_path)
	if not is_instance_valid(nav_cache_tscn):
		dprint('[WARNING] Failed to load cached nav mesh .tscn (<%s>)' % [ nav_cached_path ], 'navcache_load_cached')
		return

	# Get target node
	var nav_mesh_inst_node = level.get_node('Navigation/NavigationMeshInstance')
	if not is_instance_valid(nav_mesh_inst_node):
		dprint('[WARNING] Failed to walk node tree to NavigationMeshInstance child', 'navcache_load_cached')
		return
...
func navcache_save_cached() -> bool:
	...
	var level = get_level_node_to_process()
	if not is_instance_valid(level):
		dprint('[WARNING] Failed to get dereferenced copy of Level parent node', 'navcache_save_cached')
		return false

	dprint('Passing navigation mesh to save handler', 'navcache_save_cached')
	...
	if is_instance_valid(mesh_inst) and mesh_inst is NavigationMeshInstance:
		...
		dprint('[WARNING] Failed to get/validate NavigationMeshInstance node in level scene.', 'navcache_save_cached')
		return false
...
func on_UI_build_setting_update(value, setting: int) -> void:
	dprint('Received params { setting: %s, value: %s } ' % [ setting, value ], 'on:UI_build_setting_update')
	match setting:
				...
				if value >= 0 and value <= 2: # GENMODES.size():
					match value:
						0:
							...
							dprint('gen_mode updated to STATIC_COLLIDERS from settings UI', 'on_UI_build_setting_update')
						1:
							gen_mode = GENMODES.MESH_INSTANCE
							dprint('gen_mode updated to MESH_INSTANCE from settings UI', 'on_UI_build_setting_update')
						2:
							gen_mode = GENMODES.STATIC_COLLIDERS | GENMODES.MESH_INSTANCE
							dprint('gen_mode updated to STATIC_COLLIDERS | MESH_INSTANCE from settings UI', 'on_UI_build_setting_update')
				else:
...
func on_UI_build_navmesh() -> void:
	dprint('Received signal to build navmesh', 'on_UI_build_navmesh')
	rebuild_nav()

func on_UI_save_navmesh() -> void:
	dprint('Received signal to save navmesh', 'on_UI_save_navmesh')
	navcache_save_cached()

func on_UI_load_navmesh() -> void:
	dprint('Received signal to restore navmesh', 'on_UI_load_navmesh')
