...
const debug = true
const dprint_base_ctx := 'ObjExporter'
...
const DEFAULT_SCALE_FACTOR: float = 4.5
var scale_factor := DEFAULT_SCALE_FACTOR

var mdt = MeshDataTool.new()
const MESH_UV_TRANSFORM := Vector2(1.0, -1.0)
const MESH_UV_TRANSFORM_OFFSET := Vector2(0.0, 1.0)
func process_mesh(mesh: Mesh, offset: Vector3, scale_factor: float = scale_factor, flip_uv := true) -> void:
	mdt.clear()
...
class TrenchBroomGameFolder:
	...
	const MODELS_TEX_LEAF = 'models/textures'
	...
	func _init(base_dir: String = ""):
		if base_dir == "":
			base_dir = CSquadUtil.Settings.game_dir
		
		if base_dir == null:
			push_error('Invalid trenchbroom game base directory <%s>' % [ base_dir ])
			return
			
		path = ProjectSettings.globalize_path(base_dir.simplify_path())
...
var tb_game_folder: TrenchBroomGameFolder = TrenchBroomGameFolder.new(PROTO_TRENCHBROOM_GAMEFOLDER)
...
func resolve_spatialmat_albedo(mat: SpatialMaterial) -> Image:
	dprint(' -> SpatialMaterial', 'resolve_spatialmat_albedo')
	# Save albedo tex if found
	var alb = mat.get_texture(SpatialMaterial.TEXTURE_ALBEDO)
	if is_instance_valid(alb):
		dprint('    -> Found albedo texture in base pass', 'resolve_spatialmat_albedo')
		return alb.get_data()
	else:
		dprint('    SpatialMaterial has no albedo texture', 'resolve_spatialmat_albedo')
	
	# Check for shader albedo texture
	var color = mat.albedo_color
	if is_instance_valid(color):
		dprint('    -> Found albedo color in base pass', 'resolve_spatialmat_albedo')
		return image_from_color(color)
	else:
		dprint('    SpatialMaterial has no albedo color', 'resolve_spatialmat_albedo')
	
	dprint('    Failed to resolve albedo for SpatialMaterial', 'resolve_spatialmat_albedo')
	return null
...
func resolve_shadermat_albedo(mat: ShaderMaterial) -> Image:
	dprint(' -> ShaderMaterial', 'resolve_shadermat_albedo')
	# Save albedo tex if found
	var alb = mat.get_shader_param('albedoTex')
	if is_instance_valid(alb):
		dprint('    -> Found albedo texture shader uniform in base pass', 'resolve_shadermat_albedo')
		return alb.get_data()
	else:
		dprint('    SpatialMaterial has no albedo texture shader uniform', 'resolve_shadermat_albedo')
	
	# Check for shader albedo texture
	var color = mat.get_shader_param('albedo')
	if is_instance_valid(color):
		dprint('    -> Found albedo color shader uniform in base pass', 'resolve_shadermat_albedo')
		return image_from_color(color)
	else:
		dprint('    SpatialMaterial has no albedo color shader uniform', 'resolve_shadermat_albedo')
			
	dprint('    Failed to resolve albedo for ShaderMaterial', 'resolve_shadermat_albedo')
	return null
...
func resolve_mat_albedo(mat: Material) -> Image:
	var resolved := false
	var img: Image
	dprint('Entering Material Checks', 'resolve_mat_albedo')
	...
	if resolved:
		...
		dprint(' -> Default', 'resolve_mat_albedo')
		dprint('    (Fallback solid color not implemented)', 'resolve_mat_albedo')
		return null
...
func save_material_png(mat: Material, obj_name: String, mat_path: String = "") -> int:
	if mat_path == "":
		...
		mat_path = tb_game_folder.models_tex_dir.plus_file(
				matpath.mat_export_name_for_obj(mat, obj_name))
		dprint('mat_path not passed, computed path: <%s>' % [ mat_path ], 'save_material_png')
	else:
		dprint('Saving to passed mat_path: <%s>' % [ mat_path ], 'save_material_png')
		

	if not AssertParentDirExists(mat_path):
		dprint('Failed to create output directory <%s>' % [ mat_path ], 'save_material_png')
		return ERR_CANT_CREATE
	
	var img : Image = resolve_mat_albedo(mat)
	if is_instance_valid(img):
		dprint('Writing material png to <%s>' % [ mat_path ], 'save_material_png')
...
func get_safe_mat_export_name_for_obj(mat: Material, object_name: String) -> String:
	...
	if respath_match_idx > 0:
		...
		dprint('Found existing mtlpath <%s> for material resource path <%s>' % [ 
	...
	if not check_conflict_mtlpath(mtlpath, mat.resource_path):
		dprint('<%s> defined with matching resource path, or not yet used.' % [ mtlpath ], 'get_safe_mat_export_name_for_obj')
		return mtlpath
		
	# Else begin incrementing on end until save name
	dprint('Found existing texture resource registered under %s, iterating number prefixes until new one found' % [ 
				mtlpath], 'get_safe_mat_export_name_for_obj')
	var new_name = mtlpath
	var inc = 1
	while true:
		new_name = '%s_%d' % [ object_name, inc ]
		dprint(' -> %s' % [ new_name ], 'get_safe_mat_export_name_for_obj')
...
func save_meshes_to_obj(meshes: Array, object_name: String, obj_path: String = ""):
	if obj_path == "":
		obj_path = tb_game_folder.models_dir.plus_file(object_name + ".obj")
		
	if not AssertParentDirExists(obj_path):
		dprint('Failed to create output directory, exiting early.', 'save_meshes_to_obj')
		return

	if typeof(meshes) == TYPE_ARRAY and meshes.size() > 0:
		dprint('Building object from %s meshes' % [ meshes.size() ], 'save_meshes_to_obj')
	else:
		push_error('meshes argument is invalid or empty array.')
		return

	dprint('Using scaling factor %01.3f' % [ scale_factor ], 'save_meshes_to_obj')
	...
	for mesh_item in meshes:
		...
		dprint(' - %s Processing Mesh'   % [ prefix ], 'save_meshes_to_obj')
		...
		if typeof(mesh_item) == TYPE_ARRAY:
			...
			if mesh_item[MESHINFO.OVERRIDE] is Material:
				mat_override = true
				dprint('Handling override tuple.', 'save_meshes_to_obj')
				mat  = mesh_item[MESHINFO.OVERRIDE]
				
				mat_override_mtlpath = get_safe_mat_export_name_for_obj(mat, object_name)
				dprint('Saving override material with mtlpath <%s>' % [ mat_override_mtlpath ], 'save_meshes_to_obj')
				save_material_png(mat, object_name, 
						tb_game_folder.usemtl_to_global(mat_override_mtlpath)+'.png')
		...
		dprint('   %s Surface Count: %s' % [ prefix, mesh_surf_count ], 'save_meshes_to_obj')
		
		# Info comment
		output += '# Mesh %s\n' % [ prefix.substr(1, 5) ]
		DATA_FMT.OBJ % [ "%s_%s" % [ object_name, mesh_idx] ]

		process_mesh(mesh, offset, scale_factor)
		...
		for s in range(mesh_surf_count):
			...
			dprint('   %s Vert Count:    %s' % [ prefix, vert_count ], 'save_meshes_to_obj')
			...
			if mat_override:
				...
				dprint('   %s Saving material for %s' % [ prefix, mat_mtlpath ], 'save_meshes_to_obj')
				save_material_png(mat, object_name, 
					tb_game_folder.usemtl_to_global(mat_mtlpath + '.png'))
				commit_mtlpath(mat_mtlpath, mat.resource_path)

			if mat == null:
				dprint('   %s Using fallback material' % [ prefix ], 'save_meshes_to_obj')
				mat = mat_fallback
			...
			if len(mat_mtlpath) > 0:
				output += DATA_FMT.MAT % [ tb_game_folder.usemtl_path(mat_mtlpath) ]
			else:
	...
	dprint('Processed all meshes.', 'save_meshes_to_obj')

	write_file(obj_path, output)

	dprint('Export complete.', 'save_meshes_to_obj')

# (Assumes path is valid)
func write_file(path: String, content: String) -> int:
	var file: File = File.new()
	
	dprint('Opening file path for output: <%s>' % [ path ], 'write_file')
	
	var open_err := file.open(path, File.WRITE)
	if open_err != OK:
		push_warning('Error code %s opening output file <%s>' % [ open_err, path ])
		file.close()
		dprint('[WARNING] Failed to open output file <%s>, error code %s' % [ path, open_err  ], 'write_file')
		return open_err

	dprint('Saving constructed %s to <%s>' % [ path.get_extension(), path.get_extension() ], 'write_file')
