...
var extractor = preload('res://addons/csquad-util/src/extractors/AnyMeshExtractor.gd').new()
var root_node: Node
var attach_base: Node


# This exporter will be used differently than as originally intended (configurable settings loaded
# via CSquadUtil.Settings) so wrapping it in a custom builder for later
...
func assert_attach_base():
	if not is_instance_valid(attach_base):
		attach_base = CSquadUtil.editor.get_base_control()
...
func _init(_plugin = null):
	...
	if not is_instance_valid(plugin):
		if _plugin != null and is_instance_valid(_plugin):
			...
			plugin = _plugin
		elif CSquadUtil.plugin != null and is_instance_valid(CSquadUtil.plugin):
			dprint.write('Using plugin instance from singleton', 'on:init')
			plugin = CSquadUtil.plugin
...
func _on_tool_item_clicked(ub):
	...
	CSquadUtil.editor.set_main_screen_editor('3D')
...
func _on_export_complete():
	...
	DisconnectIfConnected(exporter, "export_completed", self, "_on_export_complete")
	DisconnectIfConnected(exporter, "export_progress", self, "_update_progress")
	disconnect_prompt()
	disconnect_exporter_progress()
...
func DisconnectIfConnected(obj: Object, obj_signal: String, target, target_method: String) -> void:
	if obj and target:
		if obj.is_connected(obj_signal, target, target_method):
			dprint.write('(%s).%s disconnecting from %s@%s' % [ target.get_name(), target_method, obj, obj_signal])
...
func disconnect_exporter_progress() -> void:
	...
	DisconnectIfConnected(exporter, "export_progress", self, "_update_progress")
	DisconnectIfConnected(exporter, "export_progress", progress, "_update_progress")


func disconnect_prompt() -> void:
	dprint.write('', 'disconnect_prompt')
	DisconnectIfConnected(prompt, 'confirmed', self, "_on_prompt_confirmed")
	DisconnectIfConnected(prompt, 'dir_selected', self, "_on_dir_selected")
	DisconnectIfConnected(prompt, "popup_hide", self, "_on_prompt_confirmed")
	DisconnectIfConnected(prompt, 'visibility_changed', self, "_on_prompt_visiblity_change")
...
func _update_progress(m_idx, s_idx, vert_pct, mesh_total):
	progress._update_progress(m_idx, s_idx, vert_pct, mesh_total)
...
# @NOTE: Unified both forms into main function, this is currently not used
#func export_current_scene_to_objs(_out_dir := out_dir):
#	#var exporter := preload("../../ObjExporter.gd").new()
#	var mesh_infos := (extractor as AnyMeshExtractor).resolve_meshes(root_node)
#
#	if mesh_infos.empty():
#		dprint.warning()
#
#	var mesh_resource_names := [ ]
#
#	# Manually duplicate each mesh to avoid scuffing it in the 3D view
...
#
#	prompt.hide()
#	progress.popup_centered(progress.get_viewport_rect().size)
#	yield(progress.get_tree().create_timer(0), 'timeout')
#
#	exporter.connect("export_progress", progress, "_update_progress")
#	exporter.connect("export_completed", self, "_on_export_complete")
#
#	# Append models subfolder to emulate current game folder structure: <BASE>/models/<EXPORTED>
#	exporter.save_meshes_to_obj(
#			mesh_infos,
#			scene_basename,
#			out_dir.plus_file('models').plus_file(scene_basename + '.obj'),
#			null,
#			true,
#			true)
#
